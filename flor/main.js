const canvas_TFan = document.getElementById("canvas");
...
gl_TFan.bufferData(gl_TFan.ARRAY_BUFFER, vertices_TFan, gl_TFan.STATIC_DRAW);
...
if (!gl_TFan.getProgramParameter(program_TFan, gl_TFan.LINK_STATUS)) {
  throw new Error(gl_TFan.getProgramInfoLog(program_TFan));
}
...
const positionLocation_TFan = gl_TFan.getAttribLocation(
  program_TFan,
  "aPosition",
);

const offsetLocation_TFan = gl_TFan.getUniformLocation(program_TFan, "uOffset");

const COUNT_TFan = vertices_TFan.length / 2;

const colorLocation_TFan = gl_TFan.getUniformLocation(program_TFan, "uColor");
// --------------------------------------------------
...
const BALLS_TFan = 5;
const DISTANCE_TFan = 0.2;

gl_TFan.uniform3f(colorLocation_TFan, 1.0, 0.0, 0.0);
for (let i = 0; i < BALLS_TFan; i++) {
  const angle = (i * 2 * Math.PI) / BALLS_TFan + Math.PI / 2;

  gl_TFan.uniform2f(
    offsetLocation_TFan,
    DISTANCE_TFan * Math.cos(angle),
    DISTANCE_TFan * Math.sin(angle),
  );

  gl_TFan.drawArrays(gl_TFan.TRIANGLE_FAN, 0, COUNT_TFan);
}

gl_TFan.uniform3f(colorLocation_TFan, 1.0, 0.85, 0.0);
gl_TFan.uniform2f(offsetLocation_TFan, 0.0, 0.0);
gl_TFan.drawArrays(gl_TFan.TRIANGLE_FAN, 0, COUNT_TFan);
